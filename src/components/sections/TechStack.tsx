...
export const TechStack = forwardRef<HTMLDivElement, { isActive: boolean }>((props, ref) => {
  ...
  return (
    <section ref={ref} className="absolute inset-0 flex  items-center justify-center  z-10 w-full h-screen p-4">
            ...
            <div className="grid grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-2 md:gap-6">
              {filteredTechs.map((tech) => (
                <div 
                  ...
                  className="tech-card opacity-0 flex flex-col items-center justify-center gap-2 p-4 bg-zinc-900/40 backdrop-blur-md border border-zinc-800/50 hover:border-[#00FF41] hover:bg-zinc-800/60 transition-all duration-300 hover:-translate-y-1 group"
                >
